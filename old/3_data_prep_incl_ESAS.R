...
for (sp in 1:nrow(species_list)) {
  ...
  if( species_list$euring[sp] %in% c(5910, 5920, 6000, 6020)) {
    ...
    frac_6049 <- esas_obs %>% 
      mutate(
        SpeciesCode = if_else(
          SpeciesCode %in% c(5911, 5912), 5910, SpeciesCode), # LBBG ssp to LBBG
        SpeciesCode = if_else(
          SpeciesCode %in% c(5921), 5920, SpeciesCode) # HG ssp to HG
      ) %>%
      filter(
        SpeciesCode %in% c(5910,5920,5927,5926,6000,5820,5900,6020)) %>% 
      group_by(CampaignID, date, SpeciesCode) %>% 
      summarise(
        sum_number = sum(Count), .groups = "drop"
        ) %>% 
      tidyr::pivot_wider(
        names_from = SpeciesCode, values_from = sum_number
        ) %>% 
      rename(
        sp_5910 = '5910',
        sp_5920 = '5920',
    ...
    data_6009 <- esas_obs %>% 
      ...
      dplyr::select(
        ...
        -fraction_5926,-fraction_6000,-fraction_5820,
        -fraction_5900
      ) %>%
      tidyr::pivot_longer(
